...
log_file_path = os.path.join("/raid/mpsych/OMAMA/logs", "dicom_processing.log")
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.StreamHandler(),  # This will log to stdout as well
    ],
)
...
def resize_image(pixel_array, target_size):
    ...
    # Resize image, keeping aspect ratio with ANTIALIAS to prevent distortion
...
def process_dicom_file(args):
    ...
    try:
        if os.path.isfile(file_path):
            ...
            image_laterality = metadata[
                1
            ]  # Assuming ImageLaterality is the second feature
            sop_instance_uid = metadata[
                2
            ]  # Assuming SOPInstanceUID is the third feature
...
def process_dicom_files(
    ...
    if cache_path:
        ...
        logger.info(f"Saved DataFrame to {cache_path}")
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        default="/home/ryan.zurrin001/binlink/cs438_features.pkl",
    )
    parser.add_argument(
        "--preds_path",
        type=str,
...
def main():
    try:
        ...
        logger.info(dicom_features_df.head())
